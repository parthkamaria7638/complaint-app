...
export class ComplaintsService {
  ...
  manageComplaints(JSONdata) {
    if(JSONdata.event.hasOwnProperty('direct_message_events')){
      ...
      if (this.checkMessageFormat(this.message)) {
        ...
        if (this.checkMessageFormat(this.message) != -1) {
          ...
          this.db.list('/twittercomplaints')
            .push(this.dataDB)
              .then(data => {
                ...
                this.http.post('https://mighty-meadow-68961.herokuapp.com/direct_messages', { id: this.sender, message: "Your complaint has been recorded. Once it gets approved by the manager, we'll get back to you." }, {headers:headers})
                  .subscribe(data => {
                    //console.log(data);
                  });
              })
          ...
          const headers = new Headers();
          headers.append('Content-Type', 'application/json');
          this.http.post('https://mighty-meadow-68961.herokuapp.com/direct_messages', { id: this.sender, message: "Message not formatted properly. Please refer to the instructions at https://domain.com/instructions" }, {headers:headers})
            .subscribe(data => {
              //console.log(data);
          });
        }
            ...
            if(JSONdata.event.tweet_create_events[0].user.id != "1092065744211169286") {
                ...
                this.db.list('/twittercomplaints')
                    ...
                    .then(data => {
                        ...
                        this.http.post('https://mighty-meadow-68961.herokuapp.com/reply_tweet', { id: this.tweet_id, name: this.tweeter, msg: msg }, {headers:headers})
                        .subscribe(data => {
                            //console.log(data);
                        });
                    })
  ...
  notifyUserWithTrackingLinkDM(userID: string, ID: string) {
    ...
    return this.http.post('https://mighty-meadow-68961.herokuapp.com/direct_messages', { id: userID, message: message }, {headers:headers});
  }
  ...
  notifyUserWithTrackingLinkTweet(tweetID: string, name: string, ID: string) {
    ...
    headers.append('Content-Type', 'application/json');
    const message = " Your complaint has been approved by the complaints manager. Here's the link to track it: " + "https://complaintsapp-cfa95.firebaseapp.com/track/" + ID;
    return this.http.post('https://mighty-meadow-68961.herokuapp.com/reply_tweet', { id: tweetID, name: name, msg: message }, {headers:headers}) 
 }
  ...
  notifyUserOfComplaintDeletionDM(complaint) {
    ...
    return this.http.post('https://mighty-meadow-68961.herokuapp.com/direct_messages', { id: complaint.IssuerID, message: message }, {headers:headers});
  }
  ...
  notifyUserOfComplaintDeletionTweet(complaint) {
    ...
    return this.http.post('https://mighty-meadow-68961.herokuapp.com/reply_tweet', { id: complaint.tweet_id, name: complaint.IssuerUsername, msg: message }, {headers:headers})    
 }

 sendFBMessage(id: string, message) {
    const headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('https://mighty-meadow-68961.herokuapp.com/send_fb_msg', { id: id,msg: message }, {headers:headers})    
 }

 sendMail(email, msg) {
    const headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('https://mighty-meadow-68961.herokuapp.com/sendMail', { msg: msg,email: email }, {headers:headers})    
